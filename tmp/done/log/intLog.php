...
{
    private $params;
    private $db_attribute_exclude_list = ['package', 'name', 'LOGSTART', 'LOGEND', 'LASTMODIFIED', 'filename', 'dbh', 'dbkey', '_plugindb_timestamp'];
    private $dbh;
...
//        }
        ...
        if (!isset($this->params['nofile'])) {
            if (!isset($this->params['filename'])) {
                $dir = $this->params['logdir'] ?? $lbplogdir;
                if (!is_dir($dir)) {
                    echo "Cannot determine plugin log directory. Terminating.\n";
                    exit(1);
                }
                $this->params['filename'] = $dir.'/'.currtime('filehires').'_'.$this->params['name'].'.log';
            }

            if (empty($this->params['append'])) {
                ...
                if (empty($this->dbh)) {
                    $this->dbh = self::log_db_init_database();
                }
                $this->params['dbkey'] = self::log_db_query_id($this->dbh, $this);
            }
        }
    }
    ...
    public function LOGSTART($msg = '')
    {
        ...
        $currtime = isset($this->params['addtime']) ? currtime('hrtimehires') : '';
        ...
        if (!isset($this->params['nofile'])) {
            $this->params['dbkey'] = self::log_db_logstart($this->dbh, $this);
        }
    }
    ...
    public function LOGEND($msg = '')
    {
        ...
        if (!isset($this->params['nofile'])) {
            self::log_db_logend($this->dbh, $this);
        }
    }
    ...
    public function DEB($msg)
    {
        ...
        if ($this->loglevel > 6) {
            if (isset($this->params['addtime'])) {
                $currtime = currtime('hrtimehires').' ';
            } else {
                $currtime = '';
            }
    ...
    public function CRIT($msg)
    {
        ...
        if ($this->loglevel > 1) {
            ...
            if ($this->params['loglevel'] < 6) {
                $this->params['loglevel'] = 6;
            }
        }
    }
    ...
    {
        if (isset($this->params['stdout'])) {
            fwrite(STDOUT, $msg.PHP_EOL);
        }
        if (isset($this->params['stderr'])) {
            fwrite(STDERR, $msg.PHP_EOL);
        }
        if (0 != $this->params['loglevel'] && !isset($this->params['nofile']) && '' != $this->params['filename']) {
            file_put_contents($this->params['filename'], $msg.PHP_EOL, FILE_APPEND);
        }
    }
    ...
    private function log_db_init_database()
    {
        ...
        // chown
        if ('loxberry' != posix_getpwuid(fileowner($dbfile))) {
    ...
    private function log_db_logstart($dbh, $p)
    {
        ...
        if (!isset($p->params['package'])) {
            throw new Exception('Create DB log entry: No PACKAGE defined');
        }
        if (!isset($p->params['name'])) {
            throw new Exception('Create DB log entry: No NAME defined');
        }
        if (!isset($p->params['filename'])) {
            throw new Exception('Create DB log entry: No FILENAME defined');
        }
    ...
    private function log_db_query_id($dbh, $p)
    {
        ...
        if (!isset($p->params['filename'])) {
            throw new Exception('log_db_queryid: No FILENAME defined');
        }
        if (!isset($dbh)) {
            error_log("log_db_query_id: dbh not defined\n");
    ...
    public function __destruct()
    {
        ...
        if (!isset($this->params['logend_called']) && !isset($this->params['nofile'])) {
            if (isset($this->params['dbkey']) && isset($this->params['STATUS'])) {
                if (!isset($this->dbh)) {
                    $dbh = self::log_db_init_database();
                } else {
                    $dbh = $this->dbh;
                }
                if (empty($dbh)) {
                    error_log('__destruct: dbh not defined');
